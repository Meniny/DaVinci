...
extension UIImage {
    ...
    /// :param: DaVinciDrawingClosureWithSize A drawing block that takes a `CGSize` and returns `Void`.
    ///
    /// :returns: A newly rendered image.
    public class func render(_ size: CGSize, transparency: Bool? = nil, prefersExtendedRange: Bool? = nil, DaVinciDrawingClosureWithSize: (CGSize) -> Void) -> UIImage {
        ...
        return renderer.image { context in
            DaVinciDrawingClosureWithSize(size)
        }
    }
